...
def create_project(
    ...
    rules = [x.strip() for x in scope.splitlines() if x.strip()]
    if not rules:
        raise HTTPException(400, "At least one scope entry is required.")
    if template_slug not in PROJECT_TEMPLATES:
        template_slug = "web-api"
    project = Project(name=name.strip(), scope_text="\n".join(rules), template_slug=template_slug, engagement_type=PROJECT_TEMPLATES[template_slug].get("engagement_type", "authorized_pentest"))
    db.add(project)
    db.commit()
    db.refresh(project)
    seed_builtin_skills(db)
