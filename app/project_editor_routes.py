...
async def preview_project(pid:int,request:Request,db:Session=Depends(get_db)):
    ...
    try:
        ...
        for rule in data['scope_text'].splitlines():
            rule=rule.strip()
            if not rule:continue
            try:ipaddress.ip_network(rule,strict=False);continue
            except ValueError:pass
            if not re.fullmatch(r'(?:\*\.)?[A-Za-z0-9](?:[A-Za-z0-9.-]*[A-Za-z0-9])?',rule):raise ValueError('范围支持域名、*.子域名、IP 或 CIDR，不包含 URL 路径。')
