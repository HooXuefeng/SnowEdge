...
async def replay_request(
    ...
    try:
        ...
        from .report_data import raw_http
        raw_request = ''
        try:
            sent = response.request
            raw_request = f'{sent.method} {sent.url.raw_path.decode("ascii")} HTTP/1.1\r\n' + '\r\n'.join(f'{k}: {v}' for k,v in sent.headers.multi_items()) + '\r\n\r\n' + sent.content.decode('utf-8', errors='replace')
        except (RuntimeError, AttributeError):
            pass  # A mocked or older transport may not expose the sent request.
        raw_response = f'{getattr(response,"http_version","HTTP/1.1")} {response.status_code} {getattr(response,"reason_phrase","")}\r\n' + '\r\n'.join(f'{k}: {v}' for k,v in response.headers.multi_items()) + '\r\n\r\n' + body
        evidence = Evidence(
            project_id=stored.project_id, source_type='report_replay', source_id=result.id,
            task_id=task.id,
            kind="request_replay_result",
            content=json.dumps({
                "raw_request": raw_http(raw_request),
                "raw_response": raw_http(raw_response),
                "body_truncated": len(response.content) > settings.max_response_body_bytes,
                "body_representation": "HTTP client decoded body; UTF-8 replacement for non-text bytes; credentials redacted",
                "stored_request_id": stored.id,
                "identity_id": identity.id if identity else None,
                "method": stored.method,
                "url": stored.url,
