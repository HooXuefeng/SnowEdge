...
def create_snapshot(db,project,ids,variant,include_images=False,options=None):
    if variant not in VARIANTS:raise ValueError('报告类型无效。')
    if len(ids)>200:raise ValueError('一次最多选择 200 条漏洞。')
    ...
    for row in rows:
        ...
        if include_images:
            from .report_data import evidence_rows
            chosen=evidence_rows(db,row)
            selected_ids={e['row'].parent_evidence_id or e['id']:e for e in chosen if e['include']}
            from sqlalchemy import or_
            images=db.query(EvidenceAttachment).filter(EvidenceAttachment.project_id==project.id,EvidenceAttachment.attachment_type=='screenshot',or_(EvidenceAttachment.finding_id==row.id,EvidenceAttachment.evidence_id.in_(selected_ids))).order_by(EvidenceAttachment.sort_order,EvidenceAttachment.id).all()
            for shot in images:
                from .report_data import presentation
                shot_meta=selected_ids[shot.evidence_id]['meta'] if shot.evidence_id in selected_ids else presentation(db,shot.evidence_id) if shot.evidence_id else {}
                if not shot_meta.get('include',True):continue
                try:
                ...
                item['images'].append({'evidence_id':shot.evidence_id,'label':shot_meta.get('caption',shot.label),'data':base64.b64encode(blob).decode(),'sha256':hashlib.sha256(blob).hexdigest()})
        data['findings'].append(item)
    from .report_data import enrich_snapshot
    data=enrich_snapshot(db,project,data,options)
    frozen=encoded(data)
...
def export_snapshot(data,fmt):
    if data.get('schema')=='snowedge-delivery/2':
        from .report_render import export
        return export(data,fmt)
