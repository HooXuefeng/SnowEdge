...
def candidates(value):
    from .hash_analysis.detector import analyze
    classification=analyze(value)
    if classification['kind'] in ('hash','encrypted'):return []
    result = []
    def attempt(kind, fn):
        try:
            decoded = fn()
            ...
            pass
    clean = value.strip()
    if classification['kind']=='encoding' and classification.get('decoded'):
        return [{'type':'十六进制','text':classification['decoded']}]
...
def decode(value, layers=1):
    ...
    original=analyze(value)
    if original['kind']=='token':analysis=original
    if analysis['kind']=='unknown' and choices:analysis.update(kind='encoding',label=choices[0]['type'],confidence='中',reason='已还原可读文本，后续未发现可靠编码。')
    return {'analysis':analysis,'candidates':choices,'steps':chain,'result':current,
